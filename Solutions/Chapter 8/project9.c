...
int main()
{
    ...
    while (walk && step <= 'Z')
    {
        ...
        array[currentRow][currentCol] = step;
        ...
        while (checkStep && walk)
        {
            ...
            int colSize = sizeof(array) / sizeof(array[0]);
            int rowSize = sizeof(array[0]) / sizeof(array[0][0]);

            //if not out of bounds or has not visited there
            if (array[nextRow][nextCol] == '.' && nextRow < colSize && nextCol < rowSize && nextRow >= 0 && nextCol >= 0)
            {
                currentRow = nextRow;
                currentCol = nextCol;
                checkStep = false;
