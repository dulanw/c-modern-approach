...
bool straight, flush, two_pairs, royal_flush;
...
void analyze_hand()
{
    ...
    for (int x = 0; x < NUM_CARDS && flush; x++)
    {
        //check
    ...
    if (flush)
    {
        bool ace, king, queen, jack, ten, valid = true;
        for (int x = 0; x < NUM_CARDS && valid; x++)
        {

            //a simple way to do this is check if all the cards ranks are above or equal to 8
            //since we already know they are the same suit then two cards cant be the same rank.
    ...
    for (int x = 0; x < NUM_CARDS - 1 && straight; x++)
    {
